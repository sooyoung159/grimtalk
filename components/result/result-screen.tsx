...
export function ResultScreen({ imageUrl, character, assistantText, audioUrl, recentMessages, onTalkAgain, onRestart }: { imageUrl?: string | null; character: T; assistantText: string; audioUrl?: string | null; recentMessages?: ConversationMessage[]; onTalkAgain: () => void; onRestart: () => void; }) {
  ...
  const isContinueTurn = Boolean(recentMessages && recentMessages.length > 2);

  return (
    <div className="space-y-4 animate-result-enter">
      <div className="space-y-2">
        <h2 className="text-[26px] font-bold leading-tight text-[#2F2A26]">{isContinueTurn ? '네 그림 친구가 이어서 대답했어' : '네 그림 친구가 처음 인사했어'}</h2>
      </div>

      <div className="space-y-3">
        <CapturePreviewCard imageUrl={imageUrl} emptyMessage="여기에 네 그림 친구가 나타날 거야" />
        <NanaBubble message={isContinueTurn ? '같은 그림 친구가 네 말에 이어서 답했어.' : '네 그림 친구가 방금 말을 걸었어.'} variant="cheer" size="sm" />
      </div>

      <div className="space-y-3">
        <CharacterCard character={character} assistantText={assistantText} messages={latest} turnMode={isContinueTurn ? 'continue_turn' : 'first_turn'} />
